...
if cfg().network == 'vgg16_one':
    from model import vgg19_2 as nn
    # from model import resnet as nn
    # from model import vgg_16_minggu_malam as nn
...
def train():
    ...
    model = nn.network()
    #model.load_weights('model00000030.hdf5')
    ...
    # checkpoint = callbacks.ModelCheckpoint('3dbox_weights_vgg19_v2.hdf5', monitor='val_loss', verbose=1, save_best_only=True, mode='min', period=1)
    ...
    model.compile(optimizer=minimizer,  #minimizer,
                  ...
                  loss_weights={'dimensions': 1., 'orientation': 1., 'confidence': 1.})
